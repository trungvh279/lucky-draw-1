...
export default function LuckyDraw1({
  ...
  const [phase, setPhase] = useState<Phase>(() => {
    return (isCompleted || confirmedHistory.length >= TOTAL_LD1_WINNERS)
      ? 'ready'
      : confirmedHistory.length > 0 ? 'ready' : 'intro';
  });
  ...
  const [allDone, setAllDone] = useState(isCompleted || confirmedHistory.length >= TOTAL_LD1_WINNERS);
  ...
  const finishEarly = useCallback(() => {
    ...
    exportLuckyDraw1ToExcel(updatedHistory);
    onComplete(updatedHistory.map(h => h.participant.id));
  }, [currentDrawnItems, confirmedWinnersHistory, roundConfig, onComplete, onHistoryUpdate]);
  ...
  }, [allDone, phase, confirmedWinnersHistory, startSpin, confirm, redraw, finishEarly, onBack, onGoToDraw2]);

  if (allDone) {
    const completedRoundsCount = new Set(confirmedWinnersHistory.map(w => w.round)).size;
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-center gap-6 flex-wrap">
            ...
            >
              Đã trao tặng thành công <span style={{ color: '#f0d060', fontWeight: 700 }}>{confirmedWinnersHistory.length}</span> phần quà qua {completedRoundsCount} lượt quay
            </p>
          </div>
        </div>
